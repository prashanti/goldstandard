...
def main():
	...
	inputfile=inputfile.replace("../data/MappedAnnotations/","")
	ancestorfile="../data/Ancestors-"+inputfile
	print ancestorfile
	PUT=open(ancestorfile,'w')
	classfile="../data/Classes-"+inputfile
	Out=open(classfile,'w')
	masterancestor=set()
	count=0

	### Create EQ subsumers for annotations
	for line in ANN:
		ancestorgroup=set()
